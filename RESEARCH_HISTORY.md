# Archived research history (superseded accounting)

This is the decision log for the experiments that produced the current signal.
Its historical performance numbers used the pre-v2.7 normalized execution
engine and are **not current backtest evidence**. Version 2.7 found and fixed
same-bar fill attribution, closed-market fills, omitted roll turnover,
non-self-financing contract scaling, Sortino, and CAGR-clock errors. Current
results live in `outputs/strategy_metrics.csv` and the executed notebook.

References to retired module names are historical; the deployable
implementation lives entirely in `strategy.py`. Failed and superseded
strategies remain documented here but are absent from the runtime repository.
...
rounds inspected every window. As of v2.7, no untouched holdout remains.
...
## Addendum (v2.7): execution and accounting audit supersedes v2.6 results

An adversarial implementation audit on 2026-08-03 found that v2.6 credited a
month-end-close target with the complete next close-to-close return, changed
positions on forward-filled exchange holidays, omitted continuous-contract
roll trades, and compounded returns while holding contracts-per-dollar fixed.
The last item implied free daily contract rescaling as NAV changed. Sortino
used the standard deviation of clipped returns rather than downside RMS, and
the CAGR clock omitted the first return interval.

Version 2.7 fixes those mechanics. Targets queue until each market's next raw
positive-volume session; the canonical model fills at that next close so a
new target earns none of the intervening bar; the ledger carries actual integer
contracts at $1 million; ordinary orders are participation-capped; and every
observed delivery-month switch charges two one-way roll legs. Missing held
marks or FX now fail closed. Back-adjusted differences drive P&L while raw
active-contract levels value notional. The historical optimization and
robustness CSVs were removed because their point metrics were generated by the
superseded engine and no deterministic generator or trial return paths exist
to recreate them.

The final audited ledger launches with $1 million and zero positions on
1990-01-01; earlier rows warm causal signals and risk estimates but do not
compound the live capital base. Contract quantities are frozen from the NAV
known on the decision date. Corrected selected-window results are 24.52% CAGR /
1.985 naive daily Sharpe; monthly Sharpe is 1.855 and HAC Sharpe is 1.784. A
next-open sensitivity is 25.63% / 2.052, but global inputs have dates rather
than exchange timestamps, so that ordering cannot be verified and is not
canonical. A six-month block bootstrap gives a non-selection-adjusted 95%
interval of 1.374-2.400 for monthly Sharpe and 17.62%-31.70% for CAGR. The
durable-target validation flag is therefore false. The 2005-2014 reused later
diagnostic is 14.84% / 1.248.

## Deviation log

- 2026-08-03, v2.7: replaced normalized same-bar accounting with the audited
  stateful execution ledger described above. Historical v2.6 tables below and
  above remain provenance only, not current evidence.
